...
@Processor(syncQueueNames.import_orders)
export class OrdersImportWorker extends WorkerHost {
  ...
  process(job: Job<OrdersImportPayload>) {
    return this.baseProcessor.runJob(job, (payload) =>
      this.ordersImportService.execute(payload),
    );
  }
}
